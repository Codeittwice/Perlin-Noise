...
void PerlinNoiseMap::draw(SDL_Renderer* renderer)
{
	...
	for (unsigned int i = 0; i < ROWS; i++)
	{
		for (unsigned int j = 0; j < COLLS; j++)
		{
			...
			points[i][j].pointRect.x = (ROWS + COLLS - 1 - i + j) * coefficient / 2.0f * multiply;
			points[i][j].pointRect.y = (j + i) * coefficient / 4;
...
{
	///Get Right
	if ((r + 1) <= COLLS && map[c][r + 1] == -1 && map[c][r] - 1 >= 0)
	{  
		map[c][r + 1] = map[c][r] - 1;
		map[c][r + 1] += ((rand() % 2));
		if (map[c][r + 1] > 100)
	...
	///Get Left
	if ((r - 1) >= 0 && map[c][r - 1] == -1 && map[c][r] - 1 >= 0)
	{
		map[c][r - 1] = map[c][r] - 1;
		map[c][r - 1] += (rand() % 2);
		if (map[c][r - 1] > 100)
	...
	///Get Bottom
	if ((c + 1) <= ROWS && map[c + 1][r] == -1 && map[c][r] - 1 >= 0)
	{
		map[c + 1][r] = map[c][r] - 1;
		map[c + 1][r] += (rand() % 2);
		if (map[c + 1][r] > 100)
	...
	///Get Top
	if ((c - 1) >= 0 && map[c - 1][r] == -1 && map[c][r] - 1 >= 0)
	{
		map[c - 1][r] = map[c][r] - 1;
		map[c - 1][r] += (rand() % 2);
		if (map[c - 1][r] > 100)
